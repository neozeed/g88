/* Symbol table definitions for GDB.
   ...
   $Header: /am/bigbird/home/bigbird/Usr.U6/robertb/m88k/src/g88/RCS/symtab.h,v 1.8 90/10/31 23:50:02 robertb Exp $
   $Locker:  $
...
#define BLOCK_FIRSTLINENO(bl) (bl)->firstlineno
#define BLOCK_LASTLINENO(bl) (bl)->lastlineno
#define BLOCK_COMPILED(bl) (bl)->compiled_by
#define BLOCK_LANGUAGE(bl) (bl)->language

/* Nonzero if symbols of block BL should be sorted alphabetically.  */
#define BLOCK_SHOULD_SORT(bl) ((bl)->nsyms >= 40)

#ifdef TEK_HACK
#define ADJUST_FRAME_OFFSET(val) \
{ register struct block *blk; \
     blk = get_selected_block(); \
     if (!block_gcc_compiled_p(blk) && (val) > 0) \
        (val) += 8; \
}
#endif
...
/* Fortran dimensions work in reverse of C dimensions, ie the leftmost
   dimension varies the fastest */

#define TYPE_BACK_TYPE(thistype) (thistype)->back_type
#define TYPE_LEN2(thistype) (TYPE_CODE(thistype)== TYPE_CODE_FORTRAN_ARRAY? \
fortran_length(thistype):(thistype)->length)
#endif
#endif
