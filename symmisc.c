/* Do various things to symbol tables (other than lookup)), for GDB.
   ...
   $Header: /am/bigbird/home/bigbird/Usr.U6/robertb/m88k/src/g88/RCS/symmisc.c,v 1.1 90/08/07 11:35:19 robertb Exp $
   $Locker:  $
...
#ifdef TEK_HACK
  if (outfile = fopen (filename, "r"))
    {
      fclose (outfile);
      if (!query ("Write over file \"%s\"", filename))
        ui_badnews (-1, "Symbol tables not printed.");
    }
#endif /* TEK_HACK */
  ...
  for (s = symtab_list; s; s = s->next)
    {
      ...
      ui_fprintf (outfile, "Symtab for file %s\n\n", s->filename);
      ui_fprintf (outfile, "Line table:\n\n");
      l = LINETABLE (s);
      len = l->nitems;
      for (i = 0; i < len; i++)
	ui_fprintf (outfile, " line %d at %x\n", l->item[i].line,
		 l->item[i].pc);
      ...
      for (i = 0; i < len; i++)
	{
	  ...
	  ui_fprintf (outfile, "block #%03d (object 0x%x) ", i, b);
	  ui_fprintf (outfile, "[0x%x..0x%x]", BLOCK_START (b), BLOCK_END (b));
	  if (BLOCK_SUPERBLOCK (b))
	    ui_fprintf (outfile, " (under 0x%x)", BLOCK_SUPERBLOCK (b));
	  if (BLOCK_FUNCTION (b))
	    ui_fprintf (outfile, " %s", SYMBOL_NAME (BLOCK_FUNCTION (b)));
	  ui_putc ('\n', outfile);
	  blen = BLOCK_NSYMS (b);
	  for (j = 0; j < blen; j++)
	    {
     ...
     FILE *outfile;
{
  ...
  if (SYMBOL_NAMESPACE (symbol) == LABEL_NAMESPACE)
    {
      ui_fprintf (outfile, "label %s at 0x%x", SYMBOL_NAME (symbol),
	       SYMBOL_VALUE (symbol));
      return;
    }
  if (SYMBOL_NAMESPACE (symbol) == STRUCT_NAMESPACE)
	...
	ui_fprintf (outfile, "%s ", SYMBOL_NAME (symbol));

      switch (SYMBOL_CLASS (symbol))
	{
	case LOC_CONST:
	  ui_fprintf (outfile, "const %d (0x%x),",
		   SYMBOL_VALUE (symbol), SYMBOL_VALUE (symbol));
	  break;

	case LOC_CONST_BYTES:
	  ui_fprintf (outfile, "const %d hex bytes:",
		   TYPE_LENGTH (SYMBOL_TYPE (symbol)));
	  {
	    int i;
	    for (i = 0; i < TYPE_LENGTH (SYMBOL_TYPE (symbol)); i++)
	      ui_fprintf (outfile, " %2x", SYMBOL_VALUE_BYTES (symbol) [i]);
	    ui_fprintf (outfile, ",");
	  }
	  break;

	case LOC_STATIC:
	  ui_fprintf (outfile, "static at 0x%x,", SYMBOL_VALUE (symbol));
	  break;

	case LOC_REGISTER:
	  ui_fprintf (outfile, "register %d,", SYMBOL_VALUE (symbol));
	  break;

	case LOC_ARG:
	  ui_fprintf (outfile, "arg at 0x%x,", SYMBOL_VALUE (symbol));
	  break;

	case LOC_REF_ARG:
	  ui_fprintf (outfile, "reference arg at 0x%x,", SYMBOL_VALUE (symbol));
	  break;

	case LOC_REGPARM:
	  ui_fprintf (outfile, "parameter register %d,", SYMBOL_VALUE (symbol));
	  break;

	case LOC_LOCAL:
	  ui_fprintf (outfile, "local at 0x%x,", SYMBOL_VALUE (symbol));
	  break;
	...
	case LOC_LABEL:
	  ui_fprintf (outfile, "label at 0x%x", SYMBOL_VALUE (symbol));
	  break;

	case LOC_BLOCK:
	  ui_fprintf (outfile, "block (object 0x%x) starting at 0x%x,",
		   SYMBOL_VALUE (symbol),
