/* Read dbx symbol tables and convert to internal format, for GDB.
   ...
   $Header: /home/bigbird/Usr.U6/robertb/tar/g88/RCS/dbxread.c,v 1.16 90/09/29 21:01:49 robertb Exp $
   $Locker:  $
...
static int parameter_register_number;  /* -rcb 9/25/90 */
     ...
     int instance;
{
  ...
  ui_badnews(-1,"Invalid symbol data: \"repeated\" header file that hasn't been seen before, at symtab pos %d.",
	 symnum);
}
     ...
     int typenums[2];
{
  ...
  if (filenum < 0 || filenum >= n_this_object_header_files)
    ui_badnews(-1,"Invalid symbol data: type number (%d,%d) out of range at symtab pos %d.",
     ...
     CORE_ADDR start, end;
{
  ...
  BLOCK_COMPILED (block) = type_of_compilation;
...
//  extern void close ();
  ...
  if (NUMBER_OF_SYMBOLS == 0)
	...
	free (symfile);
      ...
      free_all_symtabs ();
      free_all_psymtabs ();
      ui_fprintf(stdout, "%s has no symbol-table; symbols discarded.\n", name);
      ui_fflush (stdout);
  ...
  if (stat (name, &statbuf) == -1)
    ui_badnews(-1,"internal: error in stat of already open file.");
  ...
  if (buffer >= 0 && buffer < statbuf.st_size)
    ...
    stringtab = NULL;
  if (stringtab == NULL)
    ui_badnews(-1,"ridiculous string table size: %d bytes", name, buffer);
...
fill_symbuf ()
{
  ...
  if (nbytes <= 0)
    ui_badnews(-1,"error or end of file reading symbol table");
     ...
     int total_symbols;
{
  /* Current best guess is that there are approximately a twentieth
     ...
     oriented symbols */
  global_psymbols.size = (total_symbols / 10) + 1;  /* -rcb 8/90 */
  static_psymbols.size = (total_symbols / 10) + 1;  /* -rcb 8/90 */
...
#define	ADD_PSYMBOL_TO_LIST(NAME, NAMELENGTH, NAMESPACE, CLASS, LIST, VALUE)\
  do {		        						\
...
#ifdef PROFILE_TYPES
	      if (isalpha(p[1]))
		  ui_fprintf(stdout, "Funny...LSYM with a letter that isn't a type\n");
	      autovars++;
#endif
	      continue;
	    }
...
#ifdef PROFILE_TYPES
	  if (p[1] != 'f')
	    ui_fprintf(stdout, "Funny...FUN with a letter that isn't 'F' or 'f'.\n");
	  global_funs++;
#endif
	  ...
	  switch (p[1])
	    ...
	    case 'G':
	      ...
	      continue;
            ...
            /* Fortran puts out N_GSYM typedefs.  Ignore them here. */
            case 't':
              continue;
	    ...
	    default:
		 ...
		 of structure symbols.  We can ignore both.  */
	      if (p[1] != 'V' && p[1] != '('
		  && (p[1] < '0' || p[1] > '9'))
		ui_badnews(1,"Internal error: Unexpected debugging symbol type '%c' at symnum %d.\n",
		       p[1], symnum);
	      continue;
	    }
		...
		if (dependencies_used >= dependencies_allocated)
		  {
		    ...
		    bcopy (orig, dependency_list,
			    ...
			    * sizeof (struct partial_symtab *)));
#ifdef DEBUG_INFO
		    ui_fprintf (stderr, "Had to reallocate dependency list.\n");
		    ui_fprintf (stderr, "New dependencies allocated: %d\n",
			     dependencies_allocated);
#endif
		  }
	      }
	    else
	      ui_badnews(-1,"Invalid symbol data: \"repeated\" header file not previously seen, at symtab pos %d.",
		     symnum);
	  }
	  continue;
	...
	default:
	  ...
	  ui_badnews(1,"Bad symbol type 0x%x encountered in gdb scan", bufp->n_type);
	}
    }
...
#include "stab.def"
	...
	for (j = 0; j < (sizeof (tmp_list) / sizeof (struct xyzzy)) - 1; j++)
	  if (tmp_list[j].symnum == i)
	    break;
	ui_fprintf(stdout, "Symbol \"%s\" (0x%x) occured %d times.\n",
		tmp_list[j].name, i, profile_types[i]);
      }
    ui_fprintf(stdout, "Auto vars (under LSYM): %d\n", autovars);
    ui_fprintf(stdout, "Global funs (under FUN): %d\n", global_funs);
  }
#endif
}
...
static void
psymtab_to_symtab_1 (pst, desc, stringtab, sym_offset)
     struct partial_symtab *pst;
     int desc;
     char *stringtab;
     int sym_offset;
{
  ...
  if (pst->readin)
    {
      ui_fprintf (stderr, "Psymtab for %s already read in.  Shouldn't happen.\n",
	       pst->filename);
      return;
    }
...
//  extern void close ();
  ...
  if (!name)
    ui_badnews(-1,"No symbol file currently specified; use command symbol-file");
  ...
  if (pst->ldsymlen || pst->number_of_dependencies)
    {
      ...
      if (desc < 0)
	ui_badnews(-1,"Symbol file not readable");
	...
	stringtab = NULL;
      if (stringtab == NULL)
	ui_badnews(-1,"ridiculous string table size: %d bytes", name, stsize);
     ...
     int offset;
{
  ...
  if ((unsigned char) bufp->n_type != N_SO)
    ui_badnews(1,"First symbol in segment of executable not a source symbol");
     ...
     char *name;
{
  ...
  if (last_source_file == 0 && type != N_SO)
	...
	return;
      ...
      ui_badnews(-1,"Invalid symbol data: does not start by identifying a source file.");
    }
  ...
  switch (type)
      ...
      if (context_stack_depth > 0)
	{
      ...
      if (context_stack_depth != 0)
	ui_badnews(-1,"Invalid symbol data: unmatched N_LBRAC before symtab pos %d.",
	       symnum);
    ...
    case N_RBRAC:
	 ...
	 context that was started with N_LBRAC.  */
      new = &context_stack[--context_stack_depth];
      if (desc != new->depth)
	ui_badnews(-1,"Invalid symbol data: N_LBRAC/N_RBRAC symbol mismatch, symtab pos %d.", symnum);
      ...
      if (type_of_compilation == gcc_compiled)
#endif
...
#ifdef VARIABLES_INSIDE_BLOCK
      if (local_symbols &&
	  context_stack_depth > (type_of_compilation==gcc_compiled ? 1 : 0))
#else
    ...
    case N_BCOMM:
      if (common_block)
	ui_badnews(-1,"Invalid symbol data: common within common at symtab pos %d",
	       symnum);
...
//  extern void close ();
  ...
  if (arg_string == 0)
    ui_badnews(-1,"add-file takes a file name and an address");
...
#else
  /* Where people are using the 4.2 ld program, must not check for
     symsegs, because that puts random garbage at the end of
     the output file and that would trigger an error message.  */
  symseg_chain = 0;
#endif
     ...
     int desc;
{
  ...
  if (deftype == 'c')
    {
      if (*p++ != '=')
	ui_badnews(-1,"Invalid symbol data at symtab pos %d.", symnum);
      switch (*p++)
	{
	case 'r':
	  {
  ...
  switch (deftype)
    ...
    case 'p':
      ...
      if (type_of_compilation==gcc_compiled || BELIEVE_PCC_PROMOTION)
	break;
    ...
    default:
      ui_badnews(-1,"Invalid symbol data: unknown symbol-type code `%c' at symtab pos %d.", deftype, symnum);
    }
  return sym;
}

      ...
      if (TYPE_NFIELDS (*type) == 0)
	{
	  ...
	  for (ppt = file_symbols; ppt; ppt = ppt->next)
		...
		if (SYMBOL_CLASS (sym) == LOC_TYPEDEF
			...
			TYPE_CODE (*type))
		    && !strcmp (SYMBOL_NAME (sym), TYPE_NAME (*type)))
  ...
  switch ((*pp)[-1])
	...
	switch ((*pp)[0])
	  ...
	  case 'e':
	    ...
	    break;
	  default:
	    ui_badnews(-1,"Bad type cross reference at symnum: %d.", symnum);
	  }

	to = TYPE_NAME (type) = (char *)
	  obstack_alloc (symbol_obstack,
			 (strlen (prefix) +
			  ((char *) index (*pp, ':') - (*pp)) + 1));
	...
	*dbx_lookup_type (typenums) = type;
	...
	if (*(*pp)++ != ',')
	  ui_badnews(-1,"invalid member type data format, at symtab pos %d.",
		 symnum);
	...
	*dbx_lookup_type (typenums) = type;
    ...
    case 'a':
      if (*(*pp)++ != 'r')
	ui_badnews(-1,"Invalid symbol data: unrecognized type-code `a%c' %s %d.",
    ...
    default:
      ui_badnews(-1,"Invalid symbol data: unrecognized type-code `%c' at symtab pos %d.",
	     (*pp)[-1], symnum);
    }
     ...
     register struct type *type;
{
      ...
      for (i = 1; i <= n_baseclasses; i++)
	{
	  ...
	  switch (*(*pp)++)
	    ...
	    case '1':
	      ...
	      break;
	    default:
	      ui_badnews(-1,"Invalid symbol data: bad visibility format at symtab pos %d",
		     symnum);
	    }
  ...
  while (**pp != ';')
      ...
      if (**pp == '/')
	{
	 ...
	 ui_badnews(-1,"Invalid symbol data: bad structure-type format at symtab pos %d.",
	       symnum);
      (*pp)++;			/* Skip the comma.  */
      ...
      do
	{
	  ...
	  do
		...
		(struct next_fnfield *)alloca (sizeof (struct next_fnfield));
	      ...
	      if (**pp != ':')
		ui_badnews(-1,"invalid symtab info for method at symbol number %d.",
		       symnum);
	      *pp += 1;
      ...
      if (*(*pp)++ == '%')
	{
	  ...
	  if (type == t)
		...
		if (! strncmp (TYPE_FIELD_NAME (t, i), *pp,
	      ...
	      if (i < 0)
		ui_badnews(-1,"virtual function table field not found");
#endif /* not DG_HACK */
	    }
	  else
     ...
     register struct type *type;
{
  ...
  if (*(*pp)++ != ';')
    ui_badnews(-1,"Invalid symbol data; improper format of array type decl.");
     ...
     int typenums[2];
{
  ...
  if (self_subrange)
    ui_badnews(-1,"Type defined as subrange of itself.");
  ...
  if (n2 >= MIN_OF_TYPE(char) && n3 <= MAX_OF_TYPE(char))
    ...
    ui_badnews(-1,"Ranged type doesn't fit within known sizes.");
     ...
     int end;
{
  ...
  if (end)
    {
      if (c && c != end)
	ui_badnews(-1,"Invalid symbol data: invalid character \\%03o at symbol pos %d.", c, symnum);
    }
  else
    --p;
     ...
     int end;
{
  ...
  while (**pp != end)
    {
      if (**pp != ',')
	ui_badnews(-1,"Invalid argument list: no ',', at symtab pos %d", symnum);
      *pp += 1;
