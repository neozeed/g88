...
/* $Header: /am/bigbird/home/bigbird/Usr.U6/robertb/m88k/src/g88/readline/RCS/history.c,v 1.2 89/11/30 14:42:12 andrew Exp $ */
...
/* Stop stifling the hisory.  This returns the previous amount the history
...
/* Back up history_offset to the previous history string, and return
   a pointer to that string.  If there is no previous string, return
   a NULL pointer. */
HIST_ENTRY *
previous_history ()
{
...
/* Move history_offset forward to the next item in the input_history,
   and return the a pointer to that string.  If there is no next string,
...
/* Hairy histopry expansion on text, not tokens.  This is of general
...
/* Return the event specified at TEXT + OFFSET modifying OFFSET to
   ...
   begins this specification.  So you might call this function like:
   line = get_history_event ("!echo:p", 0);  */
char *
get_history_event (string, caller_index)
     char *string;
     int *caller_index;
{
 ...
 read_which:
     ...
     the string must be found at the start of a line. */
  {
    ...
    for (index = i; string[i]; i++)
      if (whitespace (string[i]) || string[i] == '\n' || string[i] == ':' ||
    ...
    *caller_index = i;
	...
	return (entry->line);
      ...
      if (index == old_index && history_offset == old_offset)
	goto bad_search;

      if (strncmp (temp, the_history[history_offset]->line,
		   strlen (temp)) == 0)
	goto search_won;

      if (history_offset)
	history_offset--;
      old_offset = history_offset;
      old_index = index;
      goto search_again;
    }
  }
     ...
     char **output;
{
  ...
  if (string[0] == history_subst_char) {
    ...
    sprintf (format_string, "%c%c:s/",
	     history_expansion_char, history_expansion_char);
    k = strlen (format_string);

    for (i = 1; string[i] && string[i] != history_subst_char; i++);
    strncpy (format_string + k, string + 1, i - 1);
    k += (i - 1);
    format_string[k++] = '/';
    ...
    for (j = i; string[j] && string[j] != history_subst_char; j++);
    strncpy (format_string + k, string + i, j - i);
    k += (j - i);
    format_string[k++] ='/';
    ...
    strncpy (format_string + k, string + j, strlen (string) - j);
    k += (strlen (string) - j);
    format_string[k] = '\0';
 ...
 grovel:
  ...
  for (i = j = 0; i < l; i++) {
    ...
    case -3:
      ...
      /* If the history_expansion_char is followed by whitespace or newline,
	 then it is not a candidate for expansion of any kind. */
      if (whitespace (cc) || cc == '\n' || !cc)
	goto add_char;
	...
	event = get_history_event (string, &i);
      }
	...
	switch (string[i + 1]) {
	  ...
	  /* :p means make this the last executed line.  So we return an
	     error state after adding this line to the history. */
	case 'p':
	  only_printing++;
	  goto next_special;
	...
	case 's':
	  {
		...
		new_event = (char *)alloca (1 + (l_that - l_this) + l_temp);
	  ...
	  /* :# is the line so far.  Note that we have to alloca () it since
	     RESULT could be realloc ()'ed below in add_string. */
	case '#':
	hack_pound_sign:
	  if (result) {
