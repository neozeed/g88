/* Symbol table lookup for the GNU debugger, GDB.
   ...
   $Header: /am/bigbird/home/bigbird/Usr.U6/robertb/m88k/src/g88/RCS/symtab.c,v 1.25 90/06/30 17:45:35 robertb Exp $
   $Locker:  $
     ...
     char *name;
{
  ...
  for (ps = partial_symtab_list; ps; ps = ps->next)
    if (!strcmp (name, ps->filename))
      {
	if (ps->readin)
	  ui_badnews(1,"Internal: readin pst found when no symtab found.");
     ...
     int noerr;
{
  ...
  if (sym == 0 || SYMBOL_CLASS (sym) != LOC_TYPEDEF)
	...
	return builtin_type_void;
      ...
      if (noerr)
	return 0;
      ui_badnews(-1,"No type named %s.", name);
    }
     ...
     char *name;
{
  ...
  ui_badnews(-1,"No type named unsigned %s.", name);
}
     ...
     struct block *block;
{
  ...
  if (sym == 0)
    ui_badnews(-1,"No struct type named %s.", name);
  if (TYPE_CODE (SYMBOL_TYPE (sym)) != TYPE_CODE_STRUCT)
    ui_badnews(-1,"This context has class, union or enum %s, not a struct.", name);
  return SYMBOL_TYPE (sym);
}
     ...
     struct block *block;
{
  ...
  if (sym == 0)
    ui_badnews(-1,"No union type named %s.", name);
  if (TYPE_CODE (SYMBOL_TYPE (sym)) != TYPE_CODE_UNION)
    ui_badnews(-1,"This context has class, struct or enum %s, not a union.", name);
  return SYMBOL_TYPE (sym);
}
     ...
     struct block *block;
{
  register struct symbol *sym 
    = lookup_symbol (name, block, STRUCT_NAMESPACE, 0);
  if (sym == 0)
    ui_badnews(-1,"No enum type named %s.", name);
  if (TYPE_CODE (SYMBOL_TYPE (sym)) != TYPE_CODE_ENUM)
    ui_badnews(-1,"This context has class, struct or union %s, not an enum.", name);
  return SYMBOL_TYPE (sym);
}
     ...
     char *name;
{
  ...
  if (TYPE_CODE (type) != TYPE_CODE_STRUCT
    ...
    ui_badnews(-1,"Type %s is not a structure or union type",
	   TYPE_NAME (type));
  ...
  errmsg = (char *) alloca (60 + strlen (TYPE_NAME (type)) +
			    strlen (name));
  sprintf (errmsg, "Type %s %s has no component named %s",
	   (TYPE_CODE (type) == TYPE_CODE_STRUCT ? "struct" : "union"),
	   TYPE_NAME (type), name);
  ui_badnews(-1,errmsg);
}
     ...
     int via_virtual, via_public;
{
  ...
  if (offset != 0)
    {
      ui_fprintf(stdout, "type offset non-zero in lookup_basetype_type");
      offset = 0;
    }
...
#ifdef GHSFORTRAN
  b = get_current_block();
  if (b->language == language_fortran)
  	TYPE_CODE (result_type) = TYPE_CODE_FORTRAN_ARRAY;
  else
...
#ifdef GHSFORTRAN
  TYPE_BACK_TYPE (element_type) = result_type;
#endif
     ...
     int *is_a_field_of_this;
{
	  ...
	  if (ps && !ps->readin)
	    {
	      ...
	      sym = lookup_block_symbol (block, name, namespace);
	      if (!sym)
		ui_badnews(1,"Internal error: global symbol found in psymtab but not in symtab.");
	      return sym;
	    }
	}
    }
  ...
  for (ps = partial_symtab_list; ps; ps = ps->next)
	...
	sym = lookup_block_symbol (block, name, namespace);
	if (!sym)
	  ui_badnews(1,"Internal: global symbol found in psymtab but not in symtab");
	return sym;
      }
  ...
  for (ps = partial_symtab_list; ps; ps = ps->next)
	...
	sym = lookup_block_symbol (block, name, namespace);
	if (!sym)
	  ui_badnews(1,"Internal: static symbol found in psymtab but not in symtab");
	return sym;
      }
...
/* This routine used to be lookup_block_symbol.  It was renamed so that the
   new function lookup_block_symbol can call it. */

static struct symbol *
actual_lookup_block_symbol (block, name, namespace)
     register struct block *block;
     char *name;
     enum namespace namespace;
{
...
/* Look for a symbol in block BLOCK.  For Fortran symbols, allow a lower-case
   symbol to be entered in upper-case. */
...
lookup_block_symbol (block, name, namespace)
     ...
     enum namespace namespace;
{
  register struct symbol *return_value;
  register char *cp;
  register int c;
  register char *downcased_name;

  /* Lookup the symbol as entered.  If we find it, return it. */
  return_value = actual_lookup_block_symbol(block, name, namespace);
  if (return_value)
    return return_value;

  /* If the language isn't Fortran, do no further searching. */
  if (block->language != language_fortran)
    return 0;

  /* If the symbol contains upper case letters, convert them to lower case
     and try again.
     Search the symbol for upper case letters.  If there aren't any, there's
     no point in continuing (and going to the expense of calling alloca). */
  for (cp = name; *cp && (*cp<'A' || 'Z'<*cp); ++cp)
    {}
  if (!*cp)
    return 0;

  /* Make a copy of the symbol in which all upper case letters are translated
     to lower case, then lookup that new symbol. */
  downcased_name = (char *) alloca(strlen(name)+1);
  cp = downcased_name;
  while (c = *name++)
    *cp++ = 'A'<=c && c<='Z' ? c+('a'-'A') : c;
  *cp = '\0';
  return actual_lookup_block_symbol(block, downcased_name, namespace);
}
  ...
  if (!s)
    {
      ...
      if (ps && ps->readin)
	ui_badnews(1,"Internal error: pc in read in psymtab, but not in symtab.");
  ...
  if (best_symtab == 0)
    {
      ...
      /* This used to assign (alt_line-1), but this doesn't make much sense to
	 me.  -=- andrew@frip.wv.tek.com */
      value.line = alt_line;
...
/* Find the PC value for a given source file and line number.
   ...
   Returns -1 if the source file has *no* line number information.
     ...
     int line;
{
  ...
  return (index != -1) ? l->item[index].pc : 0;
}
	...
	*endptr = *startptr;
	...
	if (l->item[index+1].line == thisline + 1)
	  *endptr = l->item[index+1].pc;
	  ...
	  {
	    register CORE_ADDR k;
	    k = find_line_pc (symtab, thisline+1);
	    *endptr = k == -1 ? 0 : k;
	  }
      return 1;
    }
...
/* Given a line table and a line number, return the index into the line
   ...
   Return -1 if none is found.  The value can be zero because it is an index.
...
decend_to_first_base:
...
#ifdef TEK_HACK
		      /* If we're not to return the first line with real code, then return
			 as line number the first line of the function block. */
		      if (!funfirstline)
			values.sals[0].line = BLOCK_FIRSTLINENO (SYMBOL_BLOCK_VALUE (sym));
#endif /* TEK_HACK */
		...
		ui_badnews(-1,"that class does not have any method named %s",copy);
	    }
	  else
	    ui_badnews(-1,"no class, struct, or union named %s", copy );
	}
      /*  end of C++  */
      ...
      if (s == 0)
	{
	  ...
	  ui_badnews(-1,"No source file named %s.", copy);
	}
  ...
  *argptr = p;
  ...
  if (sym && SYMBOL_CLASS (sym) == LOC_BLOCK)
    {
      /* Arg is the name of a function.
	 Set pc to the address of the prolog for the function.
	 Then lookup symtab and line for that address.
	 The line will be wrong; it will be the last line of the preceding
	 function. */
      pc = BLOCK_START (SYMBOL_BLOCK_VALUE (sym)) + FUNCTION_START_OFFSET;
      ...
      if (funfirstline)
	{
	  /* We want to identify the source line from which the first
	     executable, non-prolog object code was generated.
	     We're looking for the first instruction following the
	     instruction prolog code, so that, for example, a breakpoint will
	     happen only after the stack frame is set up.  Find this location
	     by finding the first line of source code whose first PC is
	     greater than or equal to the PC of the beginning of the
	     function.  (This will almost always be "greater than"; "equal
	     to" will happen only if there is no prolog.) */
	  if (pc!=value.pc && value.end)
	    {
	      register int new_pc = value.end;
	      value = find_pc_line (new_pc, 0);
	      value.pc = new_pc;
	    }
	}
      else
	{
	  /* We want to identify the first line of the function.  In C, this
	     will normally be the line containing the open-brace '{'. */
	  value.line = BLOCK_FIRSTLINENO (SYMBOL_BLOCK_VALUE (sym));
	}
  ...
  if (sym)
    ui_badnews(-1,"%s is not a function.", copy);

  if ((i = lookup_misc_func (copy)) < 0)
    ui_badnews(-1,"Function %s not defined.", copy);
  else
    {
      value.symtab = 0;
      value.line = 0;
  ...
  while (i < nelts)
      ...
      if (sym_arr[i] && SYMBOL_CLASS (sym_arr[i]) == LOC_BLOCK)
	{
	  ...
	  /* If we're not to return the first line with real code, then return
	     as line number the first line of the function block. */
	  if (!funfirstline)
	    values.sals[i].line = BLOCK_FIRSTLINENO (SYMBOL_BLOCK_VALUE (sym_arr[i]));
#endif /* TEK_HACK */

	  ui_fprintf(stdout, "[%d] file:%s; line number:%d\n",
		 (i+2), values.sals[i].symtab->filename, values.sals[i].line);
	}
      else ui_fprintf(stdout, "?HERE\n");
      i++;
    }
  ...
  while (*args)
    {
      ...
      if (*arg1 && *arg1 != ' ' && *arg1 != '\t')
	ui_badnews(-1,"Arguments must be choice numbers.");

      num = atoi (args);

      if (num == 0)
	ui_badnews(-1,"cancelled");
      else if (num == 1)
	{
      ...
      if (num > nelts + 2)
	{
	  ui_fprintf(stdout, "No choice number %d.\n", num);
	}
      else
	{
	  num -= 2;
	  if (values.sals[num].pc)
	    ...
	    {
	      ui_fprintf(stdout, "duplicate request for %d ignored.\n", num);
	    }
	}
...
sources_info ()
{
  ...
  if (symtab_list == 0 && partial_symtab_list == 0)
    {
      ui_fprintf(stdout, "No symbol table is loaded.\n");
      return;
    }
     ...
     int class;
{
  ...
  if (regexp)
    if (val = (char *) re_comp (regexp))
      ui_badnews(-1,"Invalid regexp: %s", val);
...
void
global_info()
/* Print out values of all global variables.  What we'll do
 * is go through the symtab_list, getting all blockvectors and
 * printing out globals.
 */
{
  register struct blockvector *bv;
  register struct symtab *s;
      ...
      for (i = 0; i < nsyms; i++)
        {
          ...
          if (SYMBOL_CLASS (sym) == LOC_CONST
            ...
            {
              fputs_filtered (SYMBOL_NAME (sym), stdout);
              fputs_filtered (" = ", stdout);
              print_variable_value (sym, 0, stdout);
...
/* dbx-style which command.  Print the full qualification of the given
 * identifier.
 * There may be a better way to do this.
 */
static void
which_command(arg, from_tty)
  char *arg;
  int from_tty;
{
  struct symbol *sym;
  ...
  char *filename;
  char *funcname;
  register int nested_block_count = 0;
  register int i = 0;

  sym = lookup_symbol(arg, block , VAR_NAMESPACE, 0);
  if (!sym) ui_badnews(-1, "No \"%s\" in current context", arg);
  block = block_found;
 /* Now we have the block and name; we need to print filename, function name, 
  * and var name.
  */

  while (block && (! block->function)) {
      block = block->superblock;
      if (block) nested_block_count++;
  }
  if (!block) {
/* OK, it must be global.  Look through the list of symbol tables, first
 * in blockvector[0] (contains symbols visible to entire program linked
 * together), then blockvector[1] (symbols visible only to one compilation,
 * or static symbols in C).
 */
    struct symbol *sym1;
    struct blockvector *bv;
    ...
    for (s = symtab_list; s; s = s->next)
      {
        bv = BLOCKVECTOR (s);
        block = BLOCKVECTOR_BLOCK (bv, 0);
        sym1 = lookup_block_symbol (block, arg, VAR_NAMESPACE);
        if (sym1 == sym) 
	  {
	    filename = s->filename;
	    break;
    ...
    funcname = block->function->name;
    sals = decode_line_spec(funcname, 0);
    sal = sals.sals[0];
    free (sals.sals);
    filename = sal.symtab->filename;
  }
  printf_filtered("%s:", filename);
  if (funcname) printf_filtered("%s:", funcname);
  while (i < nested_block_count) printf_filtered("$b%d:", ++i);
  printf_filtered("%s\n",  arg);
}
   

/* This is a helper routine for the X interface.  It allows us to do the
 * right thing for break points set at Fortran entry statements.
 */
void
xif_find_pc_line_and_filename(pc, lineno, name)
   CORE_ADDR pc;
   int *lineno;
   char *name;
{
   ...
   sal = find_pc_line(pc, 0);
   *lineno = sal.line;
   strcpy(name, sal.symtab->filename);
}
...
#ifdef TEK_HACK
  add_info ("globals", global_info,
	    "Global variables.");
  add_com ("which", class_vars, which_command,
