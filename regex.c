/* Extended regular expression matching and search library.
   ...
   $Header: /am/bigbird/home/bigbird/Usr.U6/robertb/m88k/src/g88/RCS/regex.c,v 1.6 89/11/17 08:09:29 davidl Exp $
   $Locker:  $
     ...
     char **argv;
{
  ...
  while (1)
      ...
      if (*pat)
	{
	  ...
	  ui_fprintf(stdout, "%d allocated, %d used.\n", buf.allocated, buf.used);

	  re_compile_fastmap (&buf);
	  ui_fprintf(stdout, "Allowed by fastmap: ");
	  for (i = 0; i < (1 << BYTEWIDTH); i++)
	    if (fastmap[i]) printchar (i);
	  ui_putchar ('\n');
	}

      ui_gets (pat);	/* Now read the string to match against */

      i = re_match (&buf, pat, strlen (pat), 0, 0);
      ui_fprintf(stdout, "Match value %d.\n", i);
    }
}
     ...
     struct re_pattern_buffer *bufp;
{
  ...
  ui_fprintf(stdout, "can %s be null\n----------", bufp->can_be_null ? "" : "not");
}
#endif
